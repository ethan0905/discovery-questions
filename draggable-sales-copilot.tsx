...
import { Clock, CheckCircle, ChevronRight, GripHorizontal } from "lucide-react"

export default function DraggableSalesCopilot() {
                ...
                return (
                  <motion.div
                    ...
                    className={`p-3 rounded-md ${
                      isActive ? "bg-slate-800 border-l-4 border-green-500" : "bg-slate-800/50"
                    }`}
                  >
                    <div className="flex items-start gap-3">
                      {isActive && <ChevronRight className="h-5 w-5 text-green-400 mt-0.5 flex-shrink-0" />}
                      <div>
          ...
          <AnimatePresence>
            {isCompleted && (
              <motion.div
                ...
                transition={{ duration: 0.3 }}
                className="mt-4 p-4 bg-slate-800 border border-green-700/30 rounded-md"
              >
                <div className="flex items-center gap-3 mb-2">
                  <CheckCircle className="h-5 w-5 text-green-400" />
                  <h3 className="text-green-300 font-medium">Discovery Complete</h3>
                </div>
                <p className="text-sm text-slate-300">
                  You've covered all key discovery questions. Consider scheduling a follow-up or demo based on their
                  needs.
                </p>
                <div className="mt-3 pt-3 border-t border-slate-700 text-xs text-green-400/80">
