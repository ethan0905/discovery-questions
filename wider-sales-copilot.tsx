...
export default function WiderSalesCopilot() {
  // Custom colors
  ...
  return (
    <div className="fixed inset-0 pointer-events-none" ref={constraintsRef}>
      ...
      >
        ...
        <div
          className="bg-slate-800 p-5 flex justify-between items-center border-b border-slate-700 cursor-move"
                  ...
                  >
                    <div className="flex items-start gap-3">
                      {isActive && (
                        <>
                          {isNextStep ? (
                            <Flag className="h-6 w-6 mt-0.5 flex-shrink-0" style={{ color: nextStepOrange }} />
                          ) : (
                            <ChevronRight
          ...
          <AnimatePresence>
            {isCompleted && (
              <motion.div
                ...
                transition={{ duration: 0.3 }}
                className="mt-5 p-5 bg-slate-800 rounded-md"
                style={{ borderColor: `${customGreen}33`, borderWidth: "1px" }}
              >
                <div className="flex items-center gap-3 mb-3">
                  <CheckCircle className="h-6 w-6" style={{ color: customGreen }} />
                  <h3 className="text-lg font-medium" style={{ color: customGreen }}>
                    Discovery Complete
                  </h3>
                </div>
